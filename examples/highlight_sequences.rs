...
fn main() {
    ...
    println!("{}", seq.format_with_highlighted_basepos(Some(position)));

    // Highlight the 2nd-4th base (1-based; both-end inclusive)
    println!("\nHighlight the interval: 2nd-4th (1-based both-end inclusive) with square brackets");
    ...
    println!(
        "{}",
        seq.format_with_highlighted_base_interval(Some(&interval2))
    );
